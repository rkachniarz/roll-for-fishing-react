...
export function playGame(location, player){
    ...
    rollMessage = `You've encountered a ${fish.provideDescription()} - it required a roll of ${fish.requiredRoll} to catch.`
...
export function showHistory(player){
    return(player.fishHistory.map(({fish, rollTotal}) => `<li>${fish.provideDescription()}, roll required: ${fish.requiredRoll}, your roll: ${rollTotal}, xp gained: ${fish.difficulty}</li>`).join(''))
}
